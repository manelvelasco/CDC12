...
\title{\LARGE \bf
An Alternative Representation for discrete LTI Systems with a Time Delay: Modelling and Potential Benefits
}
...
\author{Manel Velasco and Pau Mart\'{i}% <-this % stops a space
\thanks{This work was  supported by the EVENTS (Event-Driven Embedded and Networked Control Systems) project, CICYT DPI2010-18601}% <-this % stops a space
\thanks{M. Velasco and P. Mart\'{i} are with the Automatic Control Department, Technical University of Catalonia, Pau Gargallo 5, 08028 Barcelona, Spain
        {\tt\small \{manel.velasco, pau.marti\}@upc.edu}}%
}
...
Using state-space models, standard control textbooks indicate that the simplest approach to model discrete LTI systems with a time delays is to 

For event-driven controllers, event conditions in the form of boundaries specify the rule that triggers the execution of control jobs. The resulting activation patterns, i.e., sampling sequences, are aperiodic in the general case. This paper provides insight into whether a boundary for LTI event-driven controllers can be found providing a progressive optimal sampling sequence.  In particular, the paper presents an analytical closed form for a boundary that at each controller execution permits obtaining  the next $n$ job activation times in such a way that a continuous-time quadratic cost function is minimized. Numerical examples illustrate the presented approach and assess the goodness of the optimal aperiodic sequences with respect to optimal periodic control.
